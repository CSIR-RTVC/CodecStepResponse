...
std::unique_ptr<IVideoCodecTransform> createAndInitialiseCodec(const std::string& sVideoCodec, const std::string& sVideoCodecImpl, uint32_t uiWidth, uint32_t uiHeight, double dFps, const std::vector<std::string>& videoCodecParams)
{
...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    std::unique_ptr<IVideoCodecTransform> pCodec = createAndInitialiseCodec(sVideoCodec, sVideoCodecImpl, uiWidth, uiHeight, dFps, videoCodecParams);
    if (!pCodec)
    {
    ...
    LOG(INFO) << "Read " << iCount << " frames in " << sYuvFile << " (" << elapsed_ms.count() << " ms)";
  }
